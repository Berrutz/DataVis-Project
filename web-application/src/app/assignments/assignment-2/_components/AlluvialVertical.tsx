  ...
  useEffect(() => {
    ...
    sankeyData.nodes.forEach((node) => {
      if (!sortedEnergySources.includes(node.name)) {
        ...
        node.y1 = (node.y1 ?? 0) + additionalWidth;
      }
    });
    ...
    svg
      ...
      .attr('x', (d) => d.x0 ?? 0) // Use x0 for vertical positioning
      .attr('y', (d) => d.y0 ?? 0) // Use y0 for horizontal positioning
      .attr('width', (d) => (d.x1 ?? 0) - (d.x0 ?? 0)) // Width will be determined by the node's vertical position
      .attr('height', (d) => (d.y1 ?? 0) - (d.y0 ?? 0)) // Height based on the y-values
      .attr('fill', (d) => {
...
export default AlluvialVertical;

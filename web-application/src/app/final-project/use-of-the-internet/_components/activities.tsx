...
export default function Activities() {
    ...
    const fetchData = async () => {
      try {
        ...
        const ActivitiesCsvData: ActivitiesData[] = await d3.csv(
          getStaticFile(
            'datasets/final-project/use-of-the-internet/activities/percentage_people_good_and_services.csv'
          ),
          (d: any) => ({
            forwhat: d.ForWhat,
            country: d.Country,
        ...
        const uniqueYears = Array.from(
          new Set(ActivitiesCsvData.map((d) => d.year))
        ).sort();
        const uniqueCountries = Array.from(
          new Set(ActivitiesCsvData.map((d) => d.country))
        ).sort();
  ...
  if (!selectedCountry || AllData === null || chartData === undefined) {
    return (
      <ChartContainer>
  ...
  return (
    <ChartContainer className="flex flex-col gap-10">
      ...
      <LineChart
        ...
        height={600}
        xLabel="Years"
        yLabel="Percentage of Individual"
        yLowerBound={0}
        unitOfMeasurement="%"
        ml={80}
        mb={70}
